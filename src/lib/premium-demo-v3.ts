...
const REPRESENTATIVE_BODIES: Record<string, string> = {
  ...
  academic: filler([
    "认知与学习方式（示例数据）：日主己土坐酉金见申金伤官格，说明信息的吸收更靠「用输出反向巩固」——写作、讲述、动手做样本比被动听讲更有效；紫微命宫紫微·天相组合与官禄宫廉贞·破军的搭配，指向以「先建结构再打破」为节奏的学习方式。",
    "学科族群候选（示例数据）：以「表达 × 结构」为主线，最容易形成优势的方向是语言与人文表达、社会观察与研究、跨学科整合；实务工程与应用是次要候选；纯粹的高强度理论数学不是这张盘的天然形状。以上均需要以真实成绩与兴趣验证。",
    "常见阻力与反例条件：伤官旺时易「不服权威而中途改路」，若外部环境完全强调服从则学习动力急剧下滑；建议以清晰目标 + 可选择的方法组合作为学习框架。",
  ]),
  methodology: filler([
