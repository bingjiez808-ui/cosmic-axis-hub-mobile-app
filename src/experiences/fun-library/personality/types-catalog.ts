...
export const TYPE_CATALOG: Record<string, TypeEntry> = {
  ...
  METF: {
    ...
    howYouRead: E(
      "你先画一张最粗的地图，然后一个人上路。",
      "You sketch the roughest map, then walk out alone.",
    ),
    moments: {
      decision: E("你更相信可以修订的第一步。", "You trust a first step that can be revised."),
      relations: E("你独处充电，见面时非常投入。", "Alone to recharge; fully present when you show up."),
      change: E("变化来时你在纸上写清目的地，再改路线。", "Write the destination first; edit the route later."),
    },
    oftenMisread: E("被以为高冷，其实你在专心校对自己的地图。", "Read as aloof; you are proofreading your own map."),
    gentleAdvice: E("有时可以把地图递给别人看看。", "Hand the map over now and then."),
    coRead: ["LTCO", "MEAF"],
    misRead: ["LECO", "MTCF"],
  },
  METO: {
    ...
    howYouRead: E(
      "你需要看见地形才愿意画线，也不介意重新画。",
      "You'll only draw the line once the terrain is under your feet — and don't mind redrawing.",
    ),
    moments: {
      decision: E("你先问方向，再问步数。", "First the direction, then the steps."),
      relations: E("你独自消化，然后带结论回来。", "You digest alone, then bring the conclusion back."),
      change: E("变化让你兴奋，因为地图可以更新。", "Change excites you — the map gets an upgrade."),
    },
    oftenMisread: E("被以为漂泊，其实你随手就能定位。", "Read as drifting; you can pinpoint anywhere on the fly."),
