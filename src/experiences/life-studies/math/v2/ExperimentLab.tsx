...
export function ExperimentLab({
  ...
  const dimDiffs = useMemo(() => {
    ...
    const window = points.filter((p) => p.age >= active.startAge && p.age <= active.startAge + 8);
    return LIFE_DIMENSIONS.map((d) => {
      const eff = (active.dimensionEffects[d] ?? 0) + (active.costEffects[d] ?? 0);
      return {
        d,
        before: window[0]?.dimensions[d] ?? 50,
        after:  window[window.length - 1]?.dimensions[d] ?? 50,
        delta:  eff,
      };
    });
  }, [active, points]);
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-[12px] leading-relaxed text-amber-100/85">
            {isZh
              ? "一次只调整一个变量, 观察它如何影响学业、事业、关系、家庭、财富与健康。实验结果是对照情景, 不是命运改写。"
              : "Change one variable at a time to see how it affects study, career, relationship, family, wealth, and health. It is a comparison scenario — not a rewriting of fate."}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            onClick={onUndo}
            disabled={!active}
            data-testid="experiment-undo"
            className={`min-h-9 rounded-full border px-3 text-[11px] ${
              active ? "border-amber-400/40 text-amber-100 hover:bg-amber-400/10" : "cursor-not-allowed border-white/10 text-white/30"
            }`}
          >
            {isZh ? "撤销实验" : "Undo"}
          </button>
          <button
            ...
            data-testid="experiment-compare"
            className={`min-h-9 rounded-full border px-3 text-[11px] ${
              compareMode ? "border-cyan-300/70 bg-cyan-300/15 text-cyan-50" : active ? "border-cyan-400/40 text-cyan-100 hover:bg-cyan-400/10" : "cursor-not-allowed border-white/10 text-white/30"
            }`}
          >
          ...
          <button
            ...
            className={`min-h-9 rounded-full border px-3 text-[11px] ${
              isSaved
                ...
                : active
                  ? "border-amber-300/60 bg-amber-300/10 text-amber-50 hover:bg-amber-300/20"
        ...
        {LIFE_EXPERIMENTS.map((exp) => {
          ...
          return (
            <button
              key={exp.id}
              type="button"
              onClick={() => onSelect(on ? null : exp.id)}
              aria-pressed={on}
              data-testid={`experiment-card-${exp.id}`}
              className={`min-h-[44px] text-left rounded-xl border p-3 text-[11px] transition ${
                on
                  ? "border-cyan-300/70 bg-cyan-300/10 text-cyan-50 shadow-[0_0_0_1px_rgba(103,232,249,0.35)]"
      ...
      {active && dimDiffs && (
        <div className="mt-4 rounded-xl border border-amber-400/20 bg-[#0f0f1a]/80 p-4" data-testid="experiment-result">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6">
            {dimDiffs.map(({ d, delta, before, after }) => (
...
function summarize(exp: LifeExperiment, lang: "zh" | "en"): string {
  const parts: string[] = [];
  const all = { ...exp.dimensionEffects, ...exp.costEffects };
  ...
  for (const [k, v] of sorted) {
    ...
    parts.push(`${label} ${(v as number) > 0 ? "+" : ""}${v}`);
  }
  return parts.join(" · ");
}
