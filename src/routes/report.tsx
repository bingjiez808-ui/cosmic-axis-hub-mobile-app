  ...
  const runReport = useCallback(() => {
    ...
    (async () => {
      ...
      const req = {
        ...
        gender: search.gender,
        vedic: gv
          ? `sidereal Ascendant ${gv.ascendant?.sign_en ?? "n/a"}; Moon ${gv.moon.nakshatra_en} pada ${gv.moon.pada} (lord ${gv.moon.lord}); Vimshottari dasa ${gv.vimshottari[0]?.lord ?? gv.moon.lord}; planets ${gv.planets.map((p) => `${p.name_en} ${p.sign_en} ${Math.round(p.deg_in_sign)}°`).join(", ")}`
          : undefined,
        ziwei: gz
          ? `five-elements class ${gz.five_elements_class}; body star ${gz.body}; palaces ${gz.palaces
              .map((p) => `${p.name}(${p.branch}): ${p.major_stars.map((s) => s.name).join("·") || "空宫"}`)
              .join("; ")}`
          : undefined,
      };
            ...
            <div className="order-1 min-h-0 min-w-0 overflow-y-auto overscroll-contain rounded-2xl border border-white/10 bg-white/[0.02] p-3 sm:p-4 lg:order-2">
              <div className="relative w-full text-stone-warm/40">
                <FourSystemsChart
                  ...
                  lang={lang}
                  seed={`${search.name ?? ""}|${search.date ?? ""}|${search.time ?? ""}|${search.place ?? ""}`}
                  size={Math.min(wheelSize, chartSystem === "bazi" ? 320 : 400)}
                  stageHeight={
                    Math.min(wheelSize, chartSystem === "bazi" ? 320 : 400) +
                    (chartSystem === "bazi" ? 170 : 44)
                  }
                  hideTabs
                  active={chartSystem}
