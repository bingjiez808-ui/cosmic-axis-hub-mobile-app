...
- evidence_refs 数组最多 4 条，且路径格式必须完全匹配 /^[a-z_][a-z0-9_]*(?:\\.[a-z_][a-z0-9_]*|\\[\\d+\\])*$/i（点分或方括号索引，不含空格或连字符）。对于跨体系章节（kind=cross），evidence_refs 必须来自至少两个不同 module。禁止使用不在允许模块列表内的 module 值。
